...
three{%end%}
    ...
    def test_error_line_number_module(self):
        ...
        loader = DictLoader(
            {"layout.html": "{% module Template('sub.html') %}", "sub.html": "{{1/0}}"},
            namespace={"_tt_modules": ObjectDict(Template=load_generate)},
        )
        try:
            loader.load("layout.html").generate()
            self.fail("did not get expected exception")
        except ZeroDivisionError:
            exc_stack = traceback.format_exc()
            self.assertTrue("# layout.html:1" in exc_stack)
            self.assertTrue("# sub.html:1" in exc_stack)

    def test_error_line_number_include(self):
        loader = DictLoader(
            {"layout.html": "{% include 'sub.html' %}", "sub.html": "{{1/0}}"}
        )
        try:
            loader.load("layout.html").generate()
            self.fail("did not get expected exception")
        except ZeroDivisionError:
            self.assertTrue("# sub.html:1 (via layout.html:1)" in traceback.format_exc())

    def test_error_line_number_extends_base_error(self):
        loader = DictLoader(
            {"layout.html": "{{1/0}}", "sub.html": "{% extends 'layout.html' %}"}
        )
        try:
...
{% end %}
        ...
        try:
            ...
            self.assertTrue("# sub.html:4 (via layout.html:1)" in traceback.format_exc())
